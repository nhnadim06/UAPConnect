...
from accounts.models import UnverifiedRegistration, AuthToken, User, TokenType
...
def test_register_user(client: Client):
    ...
    pending_user = UnverifiedRegistration.objects.filter(email=request_data["email"]).first()
    assert pending_user
...
def test_verify_account_valid_code(client: Client):
    pending_user = UnverifiedRegistration.objects.create(
...
def test_set_new_password_using_valid_reset_token(client: Client, user_instance):
    ...
    reset_token = AuthToken.objects.create(
        user=user_instance,
